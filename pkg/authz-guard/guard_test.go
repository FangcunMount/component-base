// Package authzguard AuthzGuard 单元测试
package authzguard
...
// TestNewAuthzGuard 测试创建 AuthzGuard
func TestNewAuthzGuard(t *testing.T) {
	tests := []struct {
		name    string
		config  Config
		wantErr bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			guard, err := NewAuthzGuard(tt.config)
			if tt.wantErr {
				assert.Error(t, err)
				assert.Nil(t, guard)
			} else {
...
func TestCheckPermission(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			guard, err := NewAuthzGuard(Config{
