...
func main() {
	...
	// 测试带键值对的输出
	log.Debugw("Debug with key-value pairs", "user", "alice", "action", "login")
	log.Infow("Info with key-value pairs", "service", "api", "endpoint", "/users")
	log.Warnw("Warn with key-value pairs", "threshold", 100, "current", 150)
	log.Errorw("Error with key-value pairs", "error", "connection refused", "retry", 3)
