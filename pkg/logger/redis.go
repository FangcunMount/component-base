...
func NewRedisHookWithConfig(config RedisHookConfig) *RedisHook {
	return NewRedisHook(config.Enabled, config.SlowThreshold)
}
...
// formatRedisCommand 格式化 Redis 命令，用于日志输出
func formatRedisCommand(cmd redis.Cmder) string {
	args := cmd.Args()
	if len(args) == 0 {
		return cmd.Name()
	}
