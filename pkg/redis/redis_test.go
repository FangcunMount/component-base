...
func TestDeleteByPatternWithDefaultUnlink(t *testing.T) {
	...
	deleted, err := DeleteByPattern(ctx, client, "tmp:*", DefaultDeleteByPatternOptions())
	if err != nil {
...
func TestDefaultDeleteByPatternOptions(t *testing.T) {
	opts := DefaultDeleteByPatternOptions()
	if opts.ScanCount != 100 {
		t.Fatalf("DefaultDeleteByPatternOptions().ScanCount = %d, want 100", opts.ScanCount)
	}
	if opts.BatchSize != 500 {
		t.Fatalf("DefaultDeleteByPatternOptions().BatchSize = %d, want 500", opts.BatchSize)
	}
	if !opts.UseUnlink {
		t.Fatalf("DefaultDeleteByPatternOptions().UseUnlink = false, want true")
	}
}
